...
<div class="container mt-4">
  <h2 class="mb-4 text-center">Modifica Bonsai</h2>

  <form action="{{ route('bonsai.update', $bonsai) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')

    <div class="mb-3">
      <label for="nome" class="form-label fs-5 fw-bold">Nome</label>
      <input type="text" name="nome" id="nome" class="form-control" value="{{ old('nome', $bonsai->nome) }}" required>
    </div>

    <div class="mb-3">
      <label for="descrizione" class="form-label fs-5 fw-bold">Descrizione</label>
      <textarea name="descrizione" id="descrizione" class="form-control" rows="3">{{ old('descrizione', $bonsai->descrizione) }}</textarea>
    </div>

    <div class="mb-3 d-flex flex-column flex-md-row gap-3">
      <div class="flex-fill">
        <label for="prezzo" class="form-label fs-5 fw-bold">Prezzo ($)</label>
        <input type="number" name="prezzo" id="prezzo" class="form-control" step="1" value="{{ old('prezzo', $bonsai->prezzo) }}" required>
      </div>
      ...
      <div class="flex-fill">
        ...
        <select name="species_id" id="species_id" class="form-select" required>
          ...
          @foreach ($species as $specie)
            <option value="{{ $specie->id }}" {{ $bonsai->species_id == $specie->id ? 'selected' : '' }}>
              {{ $specie->nome }}
            </option>
          @endforeach
        </select>
      ...
      <div class="flex-fill">
        ...
        <input type="number" name="altezza_cm" id="altezza_cm" class="form-control" step="1" min="0" value="{{ old('altezza_cm', $bonsai->altezza_cm) }}" required>
      </div>

      <div class="flex-fill">
        <label for="eta_anni" class="form-label fs-5 fw-bold">Età (anni)</label>
        <input type="number" name="eta_anni" id="eta_anni" class="form-control" step="1" min="0" value="{{ old('eta_anni', $bonsai->eta_anni) }}" required>
      </div>
    </div>
    ...
    <div class="mb-3">
      ...
      @if($bonsai->immagine)
        <div class="mt-2">
          <img src="{{ asset('storage/' . $bonsai->immagine) }}" alt="{{ $bonsai->nome }}" class="img-thumbnail" style="max-height: 150px;">
        </div>
      @endif
    </div>
        ...
        @foreach ($tipologies as $tipology)
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="tipologies[]" value="{{ $tipology->id }}" id="tipology{{ $tipology->id }}"
              {{ in_array($tipology->id, $bonsai->tipologies->pluck('id')->toArray()) ? 'checked' : '' }}>
